...
- After creating the ground truth file, we can visualize the ground truth plots using the following command:
    ...
    ```
- This will generate the plots for ground truth cases with channel labels in `<path_to_plot_dir>/ground_truth.json`. This can be used to verify the ground truth data.
...
- This will train the classifier, print the classification performance metrics and store the model with artifacts in `<path_to_model_dir>`. It also appends the timestamp to model artifacts to ensure uniqueness. The following files are stored in `<path_to_model_dir>`:
    ```
    - mea-channel-classification_<timestamp>_classification_dataset.csv: The dataset used for training the classifier.
...
- If the model is giving good results, we can take note of the model's path (file with `.joblib` extension) and use it in the next step during the generation of single Field potential signal during ECC data generation.
...
- The peaks for the `Force` are also detected using `Prominence Thresholding` derived from Guassian Fit around Histogram maximum of `Force` signal.
...
- This ground truth data will be used in final step to train the arrhythmia classification models and evaluate their performance.
...
- This script loads each case's preprocessed HDF file and generate a window plot for each contraction event sequentially. It discards first and last contraction events as most often they are incomplete.
- The user will have the option to label each contraction event individually or for entire case at once after each window plot is displayed.
- The following labeling options will be available to user:
    ```
...
- The statisitcal features should be extracted with the best window size model for the corresponding signal type.
